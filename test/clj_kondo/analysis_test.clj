...
(deftest data-readers-test
  (let [analysis (:analysis (with-in-str
                              ...
                              (clj-kondo/run! {:lint ["-"]
                                               :filename "src/data_readers.clj"
                                               :config {:analysis true}})))
        var-usgs (:var-usages analysis)]
    (assert-submaps2
     '[{:name f :from user :filename "src/data_readers.clj"
        :to :clj-kondo/unknown-namespace}
       {:fixed-arities #{3 2}, :end-row 1, :name-end-col 20, :name-end-row 1, :name-row 1, :name join, :filename "src/data_readers.clj",
        :from user, :col 4, :name-col 4, :end-col 20, :row 1, :to clojure.set}
       {:fixed-arities #{1 2}, :end-row 2, :name-end-col 56, :name-end-row 2, :name-row 2, :name join, :filename "src/data_readers.clj",
